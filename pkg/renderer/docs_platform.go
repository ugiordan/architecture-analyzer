...
func renderPlatformIndexDocPage(data map[string]interface{}) string {
	...
	webhookData := getMap(data, "webhooks")

	b.WriteString(fmt.Sprintf("# %s Platform Analysis\n\n", platformName))
	b.WriteString("*Generated by architecture-analyzer. All data produced by deterministic static analysis.*\n\n")
	...
	webhookCount := 0
	if webhookData != nil {
		webhookCount = getInt(webhookData, "total")
	}
	b.WriteString(fmt.Sprintf("| Webhooks | %d |\n", webhookCount))
	b.WriteString("\n")
...
func renderPlatformArchDocPage(data map[string]interface{}) string {
	...
	if webhookData := getMap(data, "webhooks"); webhookData != nil {
		total := getInt(webhookData, "total")
		if total > 0 {
			b.WriteString("## Webhooks\n\n")
			b.WriteString(fmt.Sprintf("**Total webhooks across platform**: %d\n\n", total))

			// Per-component breakdown
			if byComp := getMap(webhookData, "by_component"); len(byComp) > 0 {
				b.WriteString("| Component | Webhooks |\n")
				b.WriteString("|-----------|----------|\n")
				...
				for _, comp := range compNames {
					count := getInt(byComp, comp)
					b.WriteString(fmt.Sprintf("| %s | %d |\n", escapeMdCell(comp), count))
				}
				b.WriteString("\n")
			}

			platformWH := getSlice(webhookData, "platform_webhooks")
			if len(platformWH) > 0 {
...
func renderPlatformNetworkTopologyGraph(b *strings.Builder, data map[string]interface{}) {
	...
	b.WriteString("  <button data-action=\"fullscreen\" title=\"Toggle fullscreen\">Fullscreen</button>\n")
	b.WriteString("</div>\n")
	...
	b.WriteString("  <span><span class=\"swatch\" style=\"background:#27ae60\"></span> Has Ingress</span>\n")
	b.WriteString("  <span><span class=\"swatch\" style=\"background:#3498db;border:2px solid #f39c12\"></span> Has NetworkPolicy</span>\n")
	b.WriteString("  <span><span class=\"swatch\" style=\"background:#e74c3c;border-radius:2px;transform:rotate(45deg)\"></span> External</span>\n")
	...
	b.WriteString("  <span><span class=\"line-swatch\" style=\"background:#e67e22;border-top:2px dotted #e67e22;height:0\"></span> External</span>\n")
	b.WriteString("</div>\n\n")
}
