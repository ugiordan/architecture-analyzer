...
This component defines a large RBAC surface (131 diagram lines). The graph below groups roles by permission scope.
...
graph LR
    ...
    trainer_metrics_reader["trainer-metrics-reader"]:::narrow
    ...
    subj_prometheus_k8s["prometheus-k8s\nServiceAccount"]:::subject
    subj_prometheus_k8s -->|binds| trainer_metrics_reader
...
| trainer-metrics-reader | ClusterRoleBinding | trainer-metrics-reader | ServiceAccount/prometheus-k8s |
...
| kubeflow-trainer-controller-manager | ClusterRole |  | tokenreviews | create |
| kubeflow-trainer-controller-manager | ClusterRole |  | subjectaccessreviews | create |
...
| trainer-metrics-reader | ClusterRole |  |  | get |
